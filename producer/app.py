...
input_topic = app.topic('producer_input_1')
input_topic_manual = app.topic('producer_input_topic_manual_1')
input_topic_manual_no_ack = app.topic('producer_input_topic_manual_no_ack_1')
consumer_input_topic = app.topic('consumer_input_1')
consumer_output_topic = app.topic('consumer_output_1')
...
@app.agent(consumer_output_topic)
...
async def producer_agent(messages):
    async for message in messages:
        ...
        await external_consumer_agent.cast(value=message)
        logging.info(
...
async def producer_agent_manual(stream):
    async for event in stream.noack().events():
        ...
        logging.info(f'[producer] App in_transaction: {app.in_transaction}')
        await external_consumer_agent.cast(value=event.value)
        logging.info(
            f'[producer] Send to consumer, sleeping {SLEEP_INTERVAL} seconds')
        await asyncio.sleep(SLEEP_INTERVAL)
        ...
        if 'skip' in event.value.decode('utf-8') and EXCEPTION_ON_RAISE_MESSAGE:
            logging.info('[producer] Skipping message')
        else:
            event.ack()
...
async def producer_agent_manual_noack(stream):
    async for events in stream.noack().noack_take(1, 1):
        for event in events:
            ...
            if 'raise' in event.value.decode('utf-8') and EXCEPTION_ON_RAISE_MESSAGE:
                raise Exception('ERROR while  processing message')
